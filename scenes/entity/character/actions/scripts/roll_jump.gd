...
class_name RollJumpCharacterAction
extends State
...
func on_changed_direction(_last_direction: int, _new_direction: int) -> void:
	if abs(character.velocity.x) >= horizontal_movement.horizontal_max_speed * 0.5:
